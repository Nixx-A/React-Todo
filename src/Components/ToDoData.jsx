        ...
        <section className='flex justify-between items-end bg-slate-300 dark:bg-gray-100 shadow-sm overflow-x-hidden rounded-sm'>
          <ul>
            ...
            <li className='inline-block'>
              <p className='text-lg ml-2'>{task.task}</p>
            </li>
          </ul>
          <div> 
            <FiEdit onClick={() => editTodo(task.id)} className='text-lg mr-2 inline  cursor-pointer dark:text-black' />
            <AiFillDelete onClick={() => deleteTodo(task.id)} className='text-lg inline mr-1 cursor-pointer dark:text-black' />
          </div>
        </section>
      </div>
    </>
